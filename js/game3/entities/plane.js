...
export default class Plane {
  ...
  updatePositions(dt) {
    if (this.worldX >= this.gp.map.worldWidth - deviceWidth * 2) {
      ...
      if (this.status != LANDED || this.status === ENDGAME) {
        this.screenX =
          (this.gp.map.screenPlaneZoneWidth - this.w) * this.gp.physics.getPercentSpeed(this, this.speed) * 0.01;
      }
  ...
  move(dt) {
    if (this.fuel > 0) {
      if (
        this.gp.input["ArrowUp"] ||
        this.gp.input["ArrowDown"] ||
        this.gp.input["ArrowRight"] ||
        this.gp.input["ArrowLeft"] ||
        this.gp.input[" "]
      ) {
        if (!this.collision) {
          if (this.gp.input[" "]) {
